...
class DeleteAccountTableViewController: DefaultTableViewController {
    ...
    @IBAction func deleteAccount() {
        if let password = passwordTextField.text, password.count > 0 {
            ...
            Client.deleteAccount(clientAuthFactor: password, clientIdentityProvider: .synerise, authID: nil) {
                self.hideLoading()
            ...
            Client.retrieveToken { (token) in
                if token.origin == .facebook {
                    ...
                    Client.deleteAccount(clientAuthFactor: facebookToken, clientIdentityProvider: .facebook, authID: nil) {
                        self.hideLoading()
                        self.showSuccessInfo()
                    } failure: { (error) in
                        self.hideLoading()
    ...
    func deleteAccountByAppleSignIn(appleIdCredential: ASAuthorizationAppleIDCredential) {
        Client.deleteAccount(clientAuthFactor: appleIdCredential.identityToken as Any, clientIdentityProvider: .apple, authID: nil) {
            self.hideLoading()
            self.showSuccessInfo()
        } failure: { (error) in
            self.hideLoading()
