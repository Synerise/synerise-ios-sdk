...
class UpdateEmailTableViewController: DefaultTableViewController {
    ...
    @IBAction func updateEmail() {
        ...
        if let password = passwordTextField.text, password.count > 0 {
            showLoading()
            Client.requestEmailChange(email: tempEmail, password: password, externalToken: nil, authID: nil, success: {
                self.hideLoading()
            ...
            Client.retrieveToken { (token) in
                if token.origin == .facebook {
                    ...
                    Client.requestEmailChange(email: tempEmail, password: nil, externalToken: facebookToken, authID: nil, success: {
                        self.hideLoading()
                        self.showSuccessInfo()
                    }) { (error) in
                        self.hideLoading()
    ...
    func deleteAccountByAppleSignIn(appleIdCredential: ASAuthorizationAppleIDCredential) {
        ...
        Client.requestEmailChange(email: tempEmail, password: nil, externalToken: appleIdCredential.identityToken, authID: nil, success: {
            self.hideLoading()
            self.showSuccessInfo()
        }) { (error) in
            self.hideLoading()
