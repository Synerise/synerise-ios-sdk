        ...
        Client.authenticateConditionally(token: identityToken, clientIdentityProvider: .apple, authID: authID, context: context) { authResult in
            if authResult.status == .success {
                ...
                alert.addAction(UIAlertAction(title: "Client.authenticate", style: .default, handler: { _ in
                    ...
                    Client.authenticate(token: identityToken, clientIdentityProvider: .facebook, authID: authID, context: context, success: {
                        self.hideLoading()
                        self.showSuccessInfo()
                    }) { (apiError) in
                        self.hideLoading()
            ...
            if apiError.code == 404 {
                ...
                alertController.addAction(UIAlertAction(title: "Client.authenticate", style: .default, handler: { _ in
                    ...
                    Client.authenticate(token: identityToken, clientIdentityProvider: .apple, authID: authID, context: context, success: {
                        self.hideLoading()
                        self.showSuccessInfo()
                    }) { (apiError) in
                        self.hideLoading()
